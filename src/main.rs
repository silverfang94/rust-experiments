...
impl Snake {
    ...
    pub fn update(&mut self, just_eaten: bool, cols: u32, rows: u32) -> bool {
        let mut new_front: SnakePiece  = (*self.snake_parts.front().expect("Snake has no body")).clone();
...
fn main() {
    ...
    let mut window: GlutinWindow = WindowSettings::new("Snake Game", [width, height]).exit_on_esc(true).build().unwrap();
